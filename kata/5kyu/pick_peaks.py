...
def pick_peaks(arr):
    ...
    for next_element in range(1, len(arr)):
        if arr[next_element] > arr[current]:
            previous = current
            current = next_element
        elif arr[next_element] < arr[current]:
            if arr[previous] < arr[current]:
                res['pos'].append(current)
                res['peaks'].append(arr[current])
            previous = current
            current = next_element
    return res
...
print(pick_peaks([1, 2, 5, 4, 3, 2, 3, 6, 4, 1, 2, 3, 3, 4, 5, 3, 2, 1, 2, 3, 5, 5, 4,
                  3]))  # {"pos":[2,7,14,20], "peaks":[5,6,5,5]}
